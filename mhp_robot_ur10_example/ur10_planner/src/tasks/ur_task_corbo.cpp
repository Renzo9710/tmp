...
namespace mhp_planner
{
    ...
    void URTask::performTask(Environment &environment, std::string *err_msg)
    {
        ...
        // initialize path pilot (overwrite required to delete all previous)
        URBasePathPilotCORBO::Ptr path_pilot = Factory<URBasePathPilotCORBO>::instance().create("URPathPilotCORBO");
        if (path_pilot)
        {
            if (!path_pilot->fromParameterServer(_ns + "/reference_mode/path_pilot"))
            {
                PRINT_ERROR("URTask: Could not read path pilot parameters for new run.");
                return;
            }
            else
            {
                _ur_path_pilot = path_pilot;
            }
        }
        ...
        if (!obstacle_msg)
            ROS_WARN("URTask: No environment information recieved!");
        ...
        while ((Time::now() - t_start).toTime() <= tf && ok())
        {
            ...
            // measure current state
            if (!robot->output(measured_state, t_measured_state))
                PRINT_ERROR("URTask: error while retrieving plant output.");
            ...
            processRefsAndInits(sub_reference, measured_state, *optimized_states, t_measured_state);
            ...
            if (_publish_prediction_marker)
                publishPredictionMarker(*optimized_states);

            // send global path and sub reference
            if (_reference_mode == PATHPILOT)
            {
                _ur_path_pilot->publishPath();
                _ur_path_pilot->publishSubReference(sub_reference);
            }
    ...
    bool URTask::verify(const Environment &environment, std::string *msg) const
    {
        ...
        if (msg)
            *msg += environment_msg;
        if (ret_val == false)
            return false; // we need all objects in environment allocated!
    ...
    void URTask::processRefsAndInits(URBasePathPilotCORBO::SubReference &sub_reference, const Eigen::Ref<const Eigen::VectorXd> &measured_state, const TimeSeries &optimized_states,
                                     const Time &t_measured_state)
    {
        // check if ref is static for some modes and change to default if not
        if (!_state_reference_callback->isStatic())
        {
            if (_reference_mode == PATHPILOT)
            {
                PRINT_ERROR(
        ...
        if (_reference_mode == PATHPILOT)
        {
            // XRef: TR from former XRef
            // URef: -
            // SRef: -

            // extract new reference as goal state and use current measurement as start state
            Eigen::VectorXd xref;
            _state_reference_callback->getReference(Time(0.0), xref);

            // normal path pilot mode based on current state, a prediction, and the preplanned path
            sub_reference = _ur_path_pilot->sampleSubReference(measured_state, optimized_states.getValuesMatrixView(), xref);

            // assign sub reference to main reference
            TimeSeries state_ts(_state_reference_callback->getDimension());
            state_ts.add(0, sub_reference.configuration);
            _state_reference->setTrajectory(state_ts);

            // If we got a new reference the global path might have changed and so did the sub reference
            if (_new_xref)
            {
                _reinit = true;
                _new_xref = false;
    ...
    {   
        ...
        nh.getParam(ns + "/reference_mode/reference_mode_type", reference_mode);
        if (reference_mode == "PathPilot")
        {
            PRINT_INFO("URTask: Reference mode set to PathPilot.");
            _reference_mode = PATHPILOT;
            URBasePathPilotCORBO::Ptr path_pilot = Factory<URBasePathPilotCORBO>::instance().create("URPathPilotCORBO");
            if (path_pilot)
            {
                if (!path_pilot->fromParameterServer(ns + "/reference_mode/path_pilot"))
                {
                    PRINT_ERROR("URTask: Could not read path pilot parameters.");
                    return false;
                }
                else
                {
            ...
            {
                PRINT_ERROR("URTask: Could not create path pilot of unknown type.");
                return false;
            }
        }
        else if (reference_mode == "Callback")
        {
        ...
        {
            PRINT_ERROR("URTask: Unknown reference mode of type " << reference_mode);
            return false;
        }
