...
namespace mhp_robot
{
    namespace robot_set_point_manager
    {
        ...
        RobotSetPointManager::RobotSetPointManager(RobotCollision::UPtr robot_collision) : _robot_collision(std::move(robot_collision))
        {
            ...
            _qd_min_vector = Eigen::VectorXd::Zero(6);
            _qd_max_vector = Eigen::VectorXd::Zero(6);
            _qd_min_vector << -2.1, -2.1, -3.1, -3.1, -3.1, -3.1;
            _qd_max_vector << 2.1, 2.1, 3.1, 3.1, 3.1, 3.1;

            // TODO(renz) Transfer to upper function that instances the setpoint manager. Temorarily workaround
            ros::NodeHandle n;
            _obstacle_subscriber =
                n.subscribe("/robot_workspace_monitor/obstacles", 1, &RobotSetPointManager::obstacleCallback, this, ros::TransportHints().tcpNoDelay());
        }
        ...
        void RobotSetPointManager::validate()
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                _dp_table[i].resize(_waypoints[i].rows(), std::pair<int, double>(-1, std::numeric_limits<double>::max()));
            }
        ...
        bool RobotSetPointManager::getOptimalWaypoints(const objectives::BaseSetpointObjective &objective, std::vector<Eigen::VectorXd> &waypoints)
        {
            ...
            if (_selector.size() != K || _dp_table.size() != K)
                validate();
            ...
            if (waypoints.size() != K)
            {
                if (waypoints.size() == 0)
                {
                    ROS_WARN("RobotSetPointManager: Could not find any complete trajectory");
                    return false;
                }
                else if (waypoints.size() >= K / 4)
                {
                    ROS_WARN("RobotSetPointManager: Could not find complete trajectory but found %lu waypoints. Send the possible waypoints",
                             waypoints.size());
                    return true;
                }
                else
                {
                    ROS_WARN("RobotSetPointManager: Could not find complete trajectory but found %lu waypoints. Don't send the possible waypoints.",
                             waypoints.size());
                    return false;
                }
            }
        ...
        void RobotSetPointManager::obstacleCallback(const mhp_robot::MsgObstacleListConstPtr &msg)
        {
            ROS_WARN_ONCE("RobotSetpointManager: Received virtual dynamic obstacle pose.");
                                                   ...
                                                   double time) const
        {
            ...
            std::vector<bool> selector(6, true);
            for (int i = 0; i < 3; ++i)
            {
                if (qd(i) < _qd_min_vector(i) || qd(i) > _qd_max_vector(i))
                {
                    selector[i] = false;
                }
            }
            return std::all_of(selector.begin(), selector.end(), [](bool b)
                               { return b; });
            // return !(qd.minCoeff() < _qd_min || qd.maxCoeff() > _qd_max);

            // TODO(renz): Check issue with timing from ergodic trajcetory sampling (temporarily fixed by a low number of samples)
            // return true;
        }
