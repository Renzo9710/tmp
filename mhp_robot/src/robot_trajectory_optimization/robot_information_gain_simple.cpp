...
namespace mhp_robot
{
    namespace robot_trajectory_optimization
    {
        ...
        double RobotInformationGainSimple::computeCost(int k, const Eigen::Ref<const Eigen::VectorXd> &x_k)
        {
            double factor = 1;
            double gain = 0;
            if (_information_pcl.size() > 0) // be sure we already received a point cloud
            {
                ...
                // set multiplication factor to 0 if the scalara product relates to an angle outside of the FOV (horizontal -->
            ...
            {
                ROS_WARN_THROTTLE(5, "RobotInformationGainSimple: No point cloud received yet");
                return 0;
            }
        }
